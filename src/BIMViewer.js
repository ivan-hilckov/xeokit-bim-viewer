...
function createExplorerTemplate(cfg) {
  const explorerTemplate =
    ...
    <div class="tab-label-list">
      <div class="xeokit-i18n tab-label" data-xeokit-i18n="modelsExplorer.title">Models</div>
      <div class="xeokit-i18n tab-label" data-xeokit-i18n="objectsExplorer.title">Objects</div>
      <div class="xeokit-i18n tab-label" data-xeokit-i18n="classesExplorer.title">Classes</div>
      <div class="xeokit-i18n tab-label tab-label_active" data-xeokit-i18n="storeysExplorer.title">Storeys</div>
    </div>
    <div class="xeokit-tab xeokit-modelsTab">
        <div class="xeokit-tab-btn"></div>
...
class BIMViewer extends Controller {
  ...
  constructor(server, cfg = {}) {
    ...
    this._modelsExplorer = new ModelsExplorer(this, {
      ...
      loadModelsButtonElement: explorerElement.querySelector(
        ".xeokit-loadAllModels"
      ), // Can be undefined
    ...
    this._objectsExplorer = new ObjectsExplorer(this, {
      ...
      showAllObjectsButtonElement: explorerElement.querySelector(
        ".xeokit-showAllObjects"
      ),
      hideAllObjectsButtonElement: explorerElement.querySelector(
    ...
    this._classesExplorer = new ClassesExplorer(this, {
      ...
      showAllClassesButtonElement: explorerElement.querySelector(
        ".xeokit-showAllClasses"
      ),
      hideAllClassesButtonElement: explorerElement.querySelector(
    ...
    this._storeysExplorer = new StoreysExplorer(this, {
      ...
      showAllStoreysButtonElement: explorerElement.querySelector(
        ".xeokit-showAllStoreys"
      ),
      hideAllStoreysButtonElement: explorerElement.querySelector(
    ...
    if (this._enablePropertiesInspector) {
      this._propertiesInspector = new PropertiesInspector(this, {
        propertiesTabElement: inspectorElement.querySelector(
    ...
    this._sectionTool = new SectionTool(this, {
      ...
      menuButtonElement: toolbarElement.querySelector(
        ".xeokit-section-menu-button"
      ),
      menuButtonArrowElement: toolbarElement.querySelector(
        ".xeokit-section-menu-button-arrow"
      ),
      active: false,
    });
    ...
    this.viewer.scene.on("rendered", () => {
      ...
      fastNavPlugin.hideEdges =
        hideEdgesMinDrawCount <
        stats.frame.drawElements + stats.frame.drawArrays;
      fastNavPlugin.scaleCanvasResolution =
        scaleCanvasResolutionMinDrawCount <
        stats.frame.drawElements + stats.frame.drawArrays;
    });
  ...
  setConfig(name, value) {
    ...
    try {
      switch (name) {
        ...
        case "smartPivot":
          this.viewer.cameraControl.smartPivot = this._configs[name] =
            parseBool(value);
          break;

        case "saoEnabled":
          this._fastNavPlugin.saoEnabled = this._configs[name] =
            parseBool(value);
          break;
        ...
        case "saoKernelRadius":
          this.viewer.scene.sao.kernelRadius = this._configs[name] =
            parseFloat(value);
          break;

        case "saoNumSamples":
          this.viewer.scene.sao.numSamples = this._configs[name] =
            parseFloat(value);
          break;
        ...
        case "edgesEnabled":
          this._fastNavPlugin.edgesEnabled = this._configs[name] =
            parseBool(value);
          break;

        case "pbrEnabled":
          this._fastNavPlugin.pbrEnabled = this._configs[name] =
            parseBool(value);
          break;

        case "viewFitFOV":
          this.viewer.cameraFlight.fitFOV = this._configs[name] =
            parseFloat(value);
          break;

        case "viewFitDuration":
          this.viewer.cameraFlight.duration = this._configs[name] =
            parseFloat(value);
          break;

        case "perspectiveFOV":
          this.viewer.camera.perspective.fov = this._configs[name] =
            parseFloat(value);
          break;
  ...
  loadModel(modelId, done, error) {
    ...
    this._modelsExplorer.loadModel(
      modelId,
      () => {
        if (done) {
          done();
  ...
  setObjectColorSource(source) {
    switch (source) {
      ...
      default:
        source = "model";
        this.error(
          "setObjectColorSource() - Unsupported value - accepted values are 'model' and 'viewer' - defaulting to 'model'"
        );
        return;
    }
  ...
  setViewerState(viewerState, done = () => {}) {
    ...
    if (viewerState.expandObjectsTree) {
      this._objectsExplorer.expandTreeViewToDepth(
        viewerState.expandObjectsTree
      );
    }
    if (viewerState.expandClassesTree) {
      this._classesExplorer.expandTreeViewToDepth(
        viewerState.expandClassesTree
      );
    }
    if (viewerState.expandStoreysTree) {
      this._storeysExplorer.expandTreeViewToDepth(
        viewerState.expandStoreysTree
      );
    }
  ...
  setAllObjectsXRayed(xrayed) {
    if (xrayed) {
      ...
      this.viewer.scene.setObjectsXRayed(
        this.viewer.scene.xrayedObjectIds,
        false
      );
    }
  }
  ...
  getOpenTab() {
    ...
    let propertiesTab = this._inspectorElement.querySelector(
      ".xeokit-propertiesTab"
    );
  ...
  selectStorey(storeyObjectId, done) {
    ...
    if (!storeyMetaObject) {
      this.error(
        "selectStorey() - Object is not found: '" + storeyObjectId + "'"
      );
      return;
    }
    if (storeyMetaObject.type !== "IfcBuildingStorey") {
      this.error(
        "selectStorey() - Object is not an IfcBuildingStorey: '" +
          storeyObjectId +
          "'"
      );
      return;
